...
Build a CodeAnalyst release artifact:
  code-analyst-<version>.tar.gz
  code-analyst-<version>.tar.gz.sha256
  manifest.json
  install_remote.sh
EOF
}
...
init = (root / "src" / "code_analyst" / "__init__.py").read_text(encoding="utf-8")
...
if not py_match or not init_match:
    raise SystemExit("could not read version from pyproject.toml and src/code_analyst/__init__.py")
...
ARTIFACT="code-analyst-$VERSION.tar.gz"
ARTIFACT_PATH="$OUT_DIR/$ARTIFACT"
CHECKSUM_PATH="$ARTIFACT_PATH.sha256"
MANIFEST_PATH="$OUT_DIR/manifest.json"
INSTALLER_SH="$OUT_DIR/install_remote.sh"

TMP_PARENT=$(mktemp -d "${TMPDIR:-/tmp}/code-analyst-release.XXXXXX")
cleanup() {
  rm -rf "$TMP_PARENT"
}
trap cleanup EXIT INT TERM

RELEASE_ROOT="$TMP_PARENT/code-analyst-$VERSION"
...
if [ -d "$RELEASE_ROOT/docs/_generated/docdev" ]; then
  rm -rf "$RELEASE_ROOT/docs/_generated/docdev"
  mkdir -p "$RELEASE_ROOT/docs/_generated/docdev"
fi

rm -f "$ARTIFACT_PATH" "$CHECKSUM_PATH" "$MANIFEST_PATH" "$INSTALLER_SH"
tar -czf "$ARTIFACT_PATH" -C "$TMP_PARENT" "code-analyst-$VERSION"
...
printf '[code-analyst package] artifact: %s\n' "$ARTIFACT_PATH"
printf '[code-analyst package] checksum: %s\n' "$CHECKSUM_PATH"
printf '[code-analyst package] manifest: %s\n' "$MANIFEST_PATH"
printf '[code-analyst package] installer: %s\n' "$INSTALLER_SH"
